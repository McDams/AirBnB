...
## 6.3.4 prix moyen par niveau de réputation
...
## 6.4.2 taux de disponibilité par ville
...
facet_wrap(~city, scales = "free_y") +
labs(
title = "Régression linéaire simple : Prix ~ Disponibilité (availability_365)",
x = "Disponibilité sur 365 jours",
y = "Prix (€)"
) +
theme_minimal()
...
ggplot(ml_df, aes(x = availability_365, y = price)) +
  ...
  facet_wrap(~city, scales = "free_y") +
  labs(
    title = "Régression linéaire simple : Prix ~ Disponibilité (par ville)",
    x = "Disponibilité sur 365 jours (availability_365)",
    y = "Prix (€)"
  ) +
  theme_minimal()
...
```{r}
## On prend un échantillon pour éviter trop de points
set.seed(42)
...
layout(
title = "3D : Prix vs Disponibilité vs Activité (par ville)",
scene = list(
...
filter(n >= 30) %>% # évite les quartiers trop rares
group_by(city) %>%
...
geom_smooth(method = "lm", se = FALSE) +
labs(
title = "Impact de l'activité (reviews_per_month) sur le prix — 3 villes",
...
```{r}
ggplot(ml_df, aes(x = availability_365, y = price, color = city)) + geom_point(alpha = 0.15) + geom_smooth(method = "lm", se = FALSE) + labs( title = "Impact de la disponibilité (availability_365) sur le prix — comparaison 3 villes", x = "Disponibilité sur 365 jours", y = "Prix (€)" ) + theme_minimal()
...
# 3) Distribution prix (bins alignés)
...
# 6) ML perf (exemple simple linéaire sur availability_365)
...
# 7) Prix vs #logements hôte (échantillon)
set.seed(42)
...
# 8) Predicted vs Actual (réutilise m ci-dessus ou un autre modèle)
...
# 11) 3D / availability / reviews impacts (échantillons)
...
# Écrire un seul JSON aligné sur mockData.js
jsonlite::write_json(
  list(
    cityColors = NULL, # gérés en front (style)
    ...
    reviewsImpact = reviews_impact, # à générer si besoin
    data3D = data3d,
    summaryKPIs = summary_kpis
  ),
  "../src/data/mockData.json", # place le JSON accessible par le front
  auto_unbox = TRUE,
  pretty = TRUE
)
